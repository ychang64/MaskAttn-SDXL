...
| `scripts/smoke_maskattn_sdxl.py` | local SDXL Base | the same `pipe.unet` used for direct forward and sampling | `load_maskattn_pipeline()` | `--checkpoint` metadata | only `--allow-untrained-gates`; `--allow-test-only-checkpoint` is loader verification only | `runtime_audit.maskattn_forward_calls > 0` | integration or trained inference |
...
Pretrained MaskAttn gate weights are not included in this repository. Training produces `gate_final.pt` with `checkpoint_kind: trained`; qualitative generation, quality evaluation, and efficiency benchmarking require that checkpoint. Checkpoints saved without a training step are marked `test_only` and are rejected by inference except the explicit `scripts/smoke_maskattn_sdxl.py --allow-test-only-checkpoint` loader test, whose result is labelled `TEST_ONLY_UNTRAINED_CHECKPOINT`. The random-gate smoke result is labelled `UNTRAINED_INTEGRATION_ONLY`.
